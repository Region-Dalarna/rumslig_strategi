...
nvdb_sv <- nvdb_sv %>%          #ändra länstillhörighet till ditt regionid
...
nvdb_sv$lwd_scaled <- rescale(nvdb_sv$ars_dygns_trafik, to = c(1, 10), na.rm = TRUE)

library(mapview)

# Assuming nvdb_dalarna is your sf object and lwd_scaled is already calculated
map <- mapview(nvdb_sv, zcol = "lwd_scaled", lwd = "lwd_scaled", color = "black", legend = FALSE)
...
# Read your nvdb_dalarna data, assuming it's an sf object
# If it's a regular dataframe, convert it to an sf object using st_as_sf()

# Scale the 'ars_dygns_trafik' values to a range suitable for line widths
nvdb_dalarna$lwd_scaled <- rescale(nvdb_dalarna$ars_dygns_trafik, to = c(1, 10), na.rm = TRUE)

library(mapview)

# Assuming nvdb_dalarna is your sf object and lwd_scaled is already calculated
map <- mapview(nvdb_dalarna, zcol = "lwd_scaled", lwd = "lwd_scaled", color = "black", legend = FALSE)
