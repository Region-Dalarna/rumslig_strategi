...
# Hämta gpkg från:
# G:/Samhällsanalys/GIS/projekt/Regionala Noder/utdata/Kollektivtrafik
# Kollektivtrafik_7tur_helg_kväll.gpkg

# nod_kollektivtrafik_fil <- "G:/Samhällsanalys/GIS/projekt/Regionala Noder/utdata/Kollektivtrafik/Kollektivtrafik_7tur_helg_kväll.gpkg"
# 
# nod_kollektivtrafik <- st_read(nod_kollektivtrafik_fil, crs = 3006)
# 
# st_layers(nod_kollektivtrafik_fil)
# 
# mapview(nod_kollektivtrafik)
...
# ========================================== inställningar ===============================================
# dir.create("G:/Samhällsanalys/GIS/projekt/gtfs/gtfs_noder")
data_input <- "G:/Samhällsanalys/GIS/projekt/gtfs/gtfs_noder"                     # mapp för gtfs-filer
...
ar_vardag <- "20240221"                         # vi väljer en vardag för att filtrera på vardagar nedan
ar_helg <- c("20240224", "20240225")            # vi väljer en lördag och söndag för att filtrera på helger nedan
...
# 
# url_regional_2 <- "https://opendata.samtrafiken.se/gtfs/dt/dt.zip?key=8e6bdd85606943b99cfebf25f9f4353f"
# 
# GET(url_regional_2, write_disk(gtfs_regional_fil, overwrite=TRUE))           # här laddar vi ned filen
...
hpl_tatorter_alla <- avgangar_hpl %>%
  ...
  filter(!is.na(unique_id)) %>%                                              # ta bort hållplatser utanför tätorter
  st_drop_geometry() %>%                                                  # nu är vi klara med geom så vi släpper det
  select(unique_id, namn, hpl_id, stop_name, direction_id,             # välj ut relevanta kolumner och lägg i rätt 
...
hpl_tatort_riktn_aggr <- hpl_tatorter_alla %>% 
  group_by(unique_id, namn, stop_name, direction_id) %>% 
...
hpl_tatort_funk_ort_aggr <- hpl_tatort_riktn_aggr %>% 
  group_by(unique_id, stop_name) %>% 
  summarise(
    n_avgang = sum(hpl_max_avg, na.rm = TRUE),
    n_avgang_efter18 = sum(hpl_max_avg_efter18, na.rm = TRUE),
    n_avgang_helg = sum(hpl_max_avg_helg, na.rm = TRUE)) %>%  
  ungroup()

library(dplyr)

result <- hpl_tatort_funk_ort_aggr %>%
  group_by(unique_id) %>%
  arrange(desc(n_avgang), desc(n_avgang_efter18)) %>%
  slice(1) %>%
  ungroup()
...
hpl_tatort_aggr <- hpl_tatort_riktn_aggr %>%
  ...
  summarise(minst_sju_dubbelturer = ifelse(sum(minst_sju_turer, na.rm = TRUE) > 1, 1, 0),
            ...
            # ,
            # avgangar_max = hpl_max_avg) 
  ungroup()


# joinar hpl_tatort_funk_ort_aggr med hpl_tatort_aggr

noder_kollektivtrafik <- left_join(hpl_tatort_aggr, result)
...
# spara som csv
write.csv(noder_kollektivtrafik, paste0(data_input, "/funkorter_noder.csv"))
